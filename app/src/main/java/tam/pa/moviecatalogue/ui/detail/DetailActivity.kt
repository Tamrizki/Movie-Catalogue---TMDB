...
class DetailActivity : AppCompatActivity() {
    ...
    private fun setupListener() {
        binding.btnFavorite.setOnCheckedChangeListener { _, _ ->
            if (isMovie){
                movie?.let { viewModel.setFavoriteMovie(it) }
            }else{
                tv?.let { viewModel.setFavoriteTv(it) }
            }
        }
    }
        ...
        when(type){
            DataDummy.MOVIE ->{
                ...
                viewModel.findMovie(id.toInt()).observe(this, { movie ->
                    ...
                    setupView( DetailEntity(movie.title, movie.voteAverage.toString(), movie.overview, movie.backdropPath, movie.releaseDate, movie.isFavorite) )
                })
            }
            DataDummy.SERIAL_TV ->{
                isMovie = false
                viewModel.findTv( id.toInt() ).observe(this, { detail ->
                    this.tv = detail
                    setFavoriteIcon(detail.isFavorite)
                    setupView( DetailEntity(detail.name, detail.voteAverage.toString(), detail.overview, detail.backdropPath, detail.firstAirDate, detail.isFavorite) )
                })
            }
        }
        viewModel.getMovie().observe(this, {
            when(it.status){
                ...
                StatusMessage.LOADING -> {
                    // Do nothing or show loading for popular list if needed
                }
                StatusMessage.EMPTY -> {
                    binding.rvPopular.visibility = View.GONE
                    binding.textPopular.visibility = View.GONE
    ...
    private fun setupRecyclerview(list: PagedList<Movie>) {
